        ...
        {plans.map((plan) => (
          <li key={plan.id} {...stylex.props(dashboardStyles.allClassRow)}>
            <div {...stylex.props(dashboardStyles.allClassInfo)}>
              <span {...stylex.props(dashboardStyles.allClassTitle)}>
                {plan.title}
              </span>
              <span {...stylex.props(dashboardStyles.allClassMeta)}>
                {plan.plannedDurationMinutes} min · {levelLabel[plan.level]} ·{" "}
                {plan.taughtCount} times taught
              </span>
            </div>

            <Badge
              variant={plan.status}
              {...stylex.props(dashboardStyles.allClassStatus)}
            >
            ...
            <Button
              variant="ghost"
              asChild
              {...stylex.props(dashboardStyles.allClassAction)}
            >
              <Link
